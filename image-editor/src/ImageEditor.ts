...
class Tokenizer {
    ...
    public nextInt(): number {
        ...
        if (!Number.isFinite(n)) throw new Error(`Expected integer but got ${t}`);
        return n;
    }
...
class ImageEditor {
    ...
    private greyscale(image: Image): void {
        for (let x = 0; x < image.getWidth(); ++x) {
            for (let y = 0; y < image.getHeight(); ++y) {
                const color = image.getPixel(x, y);
    ...
    private emboss(image: Image): void {
        for (let x = image.getWidth() - 1; x >= 0; --x) {
            ...
            if (x > 0 && y > 0) {
                ...
                if (Math.abs(cur.r - ul.r) > Math.abs(diff)) diff = cur.r - ul.r;
                if (Math.abs(cur.g - ul.g) > Math.abs(diff)) diff = cur.g - ul.g;
                if (Math.abs(cur.b - ul.b) > Math.abs(diff)) diff = cur.b - ul.b;
            }
    ...
    private read(path: string): Image {
        const text = fs.readFileSync(path, "utf-8");
        const input = new Tokenizer(text);

        const magic = input.next();
        if (magic !== "P3") throw new Error(`Expected P3 but got ${magic}`);
    ...
    private write(path: string, image: Image): void {
        const lines: string[] = [];
        lines.push("P3");
